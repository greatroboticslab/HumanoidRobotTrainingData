...
os.makedirs("validation/validation" + vcount + "/transcripts", exist_ok=True)

src_path = "../video_processing/frames/"+args.index+"/raw_frames/frame_"+str(args.frame).zfill(4)+".jpg"

dst_dir = os.getcwd()
dst_dir = os.path.join(dst_dir, "validation/validation" + vcount + "/" + args.index)
trt_dir = os.path.join(os.getcwd(), "validation/validation" + vcount + "/transcripts/")
...
transcript_path = "../video_processing/transcripts/"+args.index+".txt"
dst_path = os.path.join(trt_dir, args.index+".txt")
